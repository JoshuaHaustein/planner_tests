...
void PushPlannerWidget::setPlanningProblem(mps::planner::util::yaml::OraclePlanningProblemDesc &desc) {
	...
	// set goal region
	// TODO set goal list to reflect these values
...
void PushPlannerWidget::buildUI() {
	...
	// Goal table
	_goals_table = new QTableWidget(0, 3);
...
void PushPlannerWidget::startPlanner() {
	...
	if (not _planner_thread.thread.joinable()) {
		...
		// create dummy goal
		mps::planner::ompl::state::goal::RelocationGoalSpecification goal_spec("DUMMY", Eigen::Vector3f(), Eigen::Quaternionf());
