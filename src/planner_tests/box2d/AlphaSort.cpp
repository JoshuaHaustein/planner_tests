...
int main(int argc, const char* const* argv) {
    ...
    sim_env::Box2DRobotVelocityControllerPtr velocity_controller;
    ...
    if (vm.count("environment")) { // checks how many times the argument 'environment' is passed
        ...
        world->loadWorld(env_path); // loads the environment
        ...
        velocity_controller = std::make_shared<sim_env::Box2DRobotVelocityController>(box2d_robot);
    ...
    } else { // if no environment is procided, we print an error message and exit
        // the log prefix is a tag printed in front of the message
        logger->logErr("Please specify an environment", log_prefix);
        return 1;
    }
    ...
    if (vm["no-gui"].as<bool>()) { // in case we have no gui, we need a full problem
        ...
        auto widget = new widget::AlphaSortWidget(world, velocity_controller, world_viewer); // see widget/AlphaTestWidget.h
